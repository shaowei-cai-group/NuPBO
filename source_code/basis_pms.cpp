...
{
	int i = 0, v = 0, c = 0, j = 0, k = 0;
	MaxSATFormula maxsatformula;
	maxsatformula.build_ins(filename, maxsatformula);

	num_vars = maxsatformula.num_vars;
	num_clauses = maxsatformula.hclause.size() + maxsatformula.sclause.size();
	top_clause_weight = maxsatformula.top_clause_weight + 1;
	...
	for (k = 0; k < maxsatformula.hclause.size(); ++k)
	{
		...
		clause_true_lit_thres[c] = temc.degree;
		...
		//cout << c << " " << top_clause_weight << " " << temc.degree << " ";
		for (j = 0; j < temc.clause.size(); ++j)
		{
			...
			clause_lit_count[c]++;
			//cout  <<cur_weight << " " << cur_lit << " ";
		}
		//cout << clause_lit_count[c] <<  endl;
	...
	for (k = 0; k < maxsatformula.sclause.size(); ++k)
	{
		...
		clause_lit_count[c] = 0;
		org_clause_weight[c] = temc.weight;
		clause_true_lit_thres[c] = temc.degree;
		if (clause_true_lit_thres[c] <= 0)
		{
			num_clauses--;
		...
		//cout << c << " " << temc.weight << " " << temc.degree << " ";
		...
		clause_lit[c][0].var_num = abs(temp_weight[0].var_num);
		clause_lit[c][0].weight = temp_weight[0].weight;
		avg_clause_coe[c] += double(clause_lit[c][0].weight);
	...
	cout << num_clauses << " " << num_hclauses << " " << num_sclauses << endl;
	delete[] temp_weight;
